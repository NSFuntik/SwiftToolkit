...
public extension String {
  ...
  func replacing(
    _ string: String,
    with other: String,
    _ options: NSString.CompareOptions? = nil) -> String {
    if let options {
  ...
  func isMember(of sequence: [String]) -> Bool {
  ...
  func equals(_ string: String, caseSensitive: Bool = false) -> Bool {
    if caseSensitive {
      return self == string
    } else {
...
public extension String {
  ...
  func format(with mask: String = "+X (XXX) XXX XX XX", symbol: Character = "X") -> String {
    ...
    func width(
      ...
      font: UIFont,
      messageUseMarkdown: Bool) -> CGFloat {
    ...
    func toAttrString(
      ...
      messageUseMarkdown: Bool = true) -> NSAttributedString {
      var str = messageUseMarkdown ? (try? AttributedString(markdown: self, options: AttributedString.MarkdownParsingOptions(
        ...
        languageCode: nil))) ?? AttributedString(self) : AttributedString(self)
