...
extension AnyFeedback {
  ...
  public static func haptic(_ events: HapticEvent...) -> Self {
    ...
    do {
      ...
      case let .url(url):
        if #available(iOS 16, macOS 13, *) {
          ...
          fatalError(
            "This should never occur since the API will be limited to iOS 16+"
          )
        }
...
public enum HapticEvent: CustomStringConvertible {
  ...
  public var description: String {
    ...
    case let .haptic(intensity, sharpness, relativeTime, duration):
      return
        "Haptic (intensity: \(intensity), sharpness: \(sharpness), time: \(relativeTime), duration: \(duration))"
    }
  }
