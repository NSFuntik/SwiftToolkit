...
extension TextSlider {
  ...
  public init<Value>(
    ...
    format: FloatingPointFormatStyle<Double>? = nil
  ...
  public init<Value>(
    ...
    in bounds: ClosedRange<Value> = 0...1,
    format: FloatingPointFormatStyle<Double>? = nil
  ) where Value: BinaryInteger, Value.Stride: BinaryFloatingPoint {
    self.init(
      value: .init(
        get: { Double(value.wrappedValue) },
        set: { value.wrappedValue = Value($0) }
...
public struct TextSlider: View {
  static let defaultFormat: FloatingPointFormatStyle<Double> = .number.precision(.fractionLength(0...1)).grouping(
    .never
  )
