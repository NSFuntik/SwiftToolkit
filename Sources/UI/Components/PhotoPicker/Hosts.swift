...
public struct PHPickerAsset: Identifiable, Hashable {
  ...
  /// Initializes a new `PHPickerAsset` instance with the specified properties.
  ///
  /// - Parameters:
  ///   - id: A unique identifier for the asset.
  ///   - image: The image representation of the asset.
...
extension NSItemProvider {
  public func loadObject<T>(of type: T.Type) async throws -> T
    ...
    fileAttributes = try await withCheckedThrowingContinuation { continuation in
      self.itemProvider.loadFileRepresentation(forTypeIdentifier: "public.item") { url, error in
        if let error {
          ...
          if let url = url {
            continuation.resume(
              returning: (itemProvider.suggestedName ?? url.lastPathComponent, url.fileSizeString ?? "")
            )
          } else {
...
extension PhotosViewController {
  fileprivate final class Coordinator: NSObject, PHPickerViewControllerDelegate,
