...
public struct CameraView: View {
  ...
  private let options: CameraViewOptions = .init()
  ...
  public var body: some View {
    ...
    .sheet(
      isPresented: $cameraNotAutorizedAfterAppear,
      onDismiss: {
        Task { @MainActor in
  ...
  private func buttonsView() -> some View {
    LazyVGrid(
      columns: .init(
        repeating: .init(.adaptive(minimum: .screenWidth / 4, maximum: .screenWidth / 2), alignment: .center),
        count: 3
      )
    ) {
      Button {
...
extension LabelStyle where Self == CameraContolLabelStyle<Material> {
  static func cameraControl<S: ShapeStyle>(tint: S, size: CGFloat) -> CameraContolLabelStyle<S> {
    CameraContolLabelStyle(tint: tint, size: size)
  }
}
...
public class Camera: NSObject, AVCapturePhotoCaptureDelegate {
  ...
  private var allCaptureDevices: [AVCaptureDevice] {
    AVCaptureDevice.DiscoverySession(
      deviceTypes: [
        .builtInTrueDepthCamera, .builtInDualCamera, .builtInDualWideCamera, .builtInWideAngleCamera,
  ...
  private func configureCaptureSession(completionHandler: (_ success: Bool) -> Void) {
    ...
    if #available(macOS 10.15, iOS 16.0, tvOS 16.0, *) {
      if let dimention = deviceInput.device.activeFormat.supportedMaxPhotoDimensions.sorted(by: {
  ...
  private var deviceOrientation: UIDeviceOrientation {
    ...
    if orientation == UIDeviceOrientation.unknown {
      orientation = UIScreen.mainScreen.orientation
    }
    return orientation
  }
  ...
  lazy var photoSettings: AVCapturePhotoSettings = {
    ...
    if #available(macOS 10.15, iOS 16.0, tvOS 16.0, *) {
      if let dimention = self.deviceInput?.device.activeFormat.supportedMaxPhotoDimensions.sorted(by: {
        ($0.width * $0.height) < ($1.width * $1.height)
      }).first {
