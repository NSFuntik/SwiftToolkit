    ...
    case .changed:
      if gesture.numberOfTouches != self.numberOfTouches {
        ...
        self.startLocation = CGPoint(
          x: self.startLocation.x + jumpDifference.width,
          y: self.startLocation.y + jumpDifference.height
        )
      ...
      self.offset = CGSize(
        width: self.location.x - self.startLocation.x,
        height: self.location.y - self.startLocation.y
      )
...
struct PinchToZoom: ViewModifier {
  ...
  func body(content: Content) -> some View {
    content
      ...
      .offset(self.offset)
      .overlay(
        PinchZoom(scale: self.$scale, anchor: self.$anchor, offset: self.$offset, isPinching: self.$isPinching)
      )
      .onTapGesture(
        count: 2,
        perform: {
